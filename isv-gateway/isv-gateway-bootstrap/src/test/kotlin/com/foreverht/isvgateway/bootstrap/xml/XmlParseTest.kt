package com.foreverht.isvgateway.bootstrap.xml

import org.junit.jupiter.api.Test
import javax.xml.parsers.DocumentBuilderFactory
import javax.xml.xpath.XPath
import javax.xml.xpath.XPathFactory
import javax.xml.xpath.XPathConstants





class XmlParseTest {

    companion object {
        private const val XML_CONTENT = "<xml><ToUserName><![CDATA[wx2547800152da0539]]></ToUserName><Encrypt><![CDATA[]]></Encrypt><AgentID><![CDATA[]]></AgentID></xml>"
    }

    @Test
    fun testParseXml(){
        val builderFactory = DocumentBuilderFactory.newInstance()
        val builder = builderFactory.newDocumentBuilder()

        val xmlDocument = builder.parse(XML_CONTENT.byteInputStream())

        val xPath: XPath = XPathFactory.newInstance().newXPath()
        val expression = "/xml/Encrypt"
        val obj =  xPath.compile(expression).evaluate(xmlDocument, XPathConstants.STRING)

        println(obj)
    }
}
